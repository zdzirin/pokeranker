...
def stats_vector(pokemon):
    ...
    return [log_normalize(stat, 255) for stat in stats] + [log_normalize(stat_total, max_stat_total)]
...
def get_all_vectors(pokemon_data):
    return [{"name": pokemon['name'], "vector": get_vector(pokemon)} for pokemon in pokemon_data]

if __name__ == "__main__":
    with open('../pokemon.json', 'r') as f:
        pokemon_data = json.load(f)
        max_stat_total = max(pokemon['stat_total'] for pokemon in pokemon_data)
        print("max_stat_total: =", max_stat_total)
        f.close()
